...
import warnings
import asyncio
from typing import Any, Callable, Iterator, List, Optional, Type, Union, Awaitable
...
class StreamingLastResponseAsyncCallbackHandler(AsyncCallbackHandler):
    ...
    def __init__(
        ...
        self.detection_queue: asyncio.Queue[str] = asyncio.Queue()
        ...
        self.callback_func: Callable[
            [Union[str, Type[StopIteration]]], Awaitable[None]
        ] = None
    ...
    async def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        if self.is_streaming_answer:
            ...
            if self.detection_queue.qsize() > self.detection_queue_size:
                ...
                last_token = await self.detection_queue.get()
            ...
            if _answer_prefix_phrase is not None:
                ...
                for _ in range(len(self._enc.encode(_answer_prefix_phrase))):
                    _token = await self.detection_queue.get()
            ...
            if not self.is_streaming_answer:
                ...
                if _answer_prefix_phrase is not None:
                    ...
                    if not self.output_stream_prefix:
                        ...
                        for _ in range(len(self._enc.encode(_answer_prefix_phrase))):
                            self._callback(await self.detection_queue.get())
                else:
                    await self.detection_queue.get()
            else:
                self._callback(await self.detection_queue.get())
